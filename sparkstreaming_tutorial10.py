...
zSetEntriesDecodedStreamingDF1.filter(col("redisEvent").contains("birthDay"))
zSetEntriesDecodedStreamingDF2.filter(~col("redisEvent").contains("birthDay"))
...
zSetEntriesDecodedStreamingDF1.withColumn("customer", from_json("redisEvent", customerJSONSchema))\
    .select(col("customer.*"))\
    .createOrReplaceTempView("Customer")

zSetEntriesDecodedStreamingDF2.withColumn("customerLocation", from_json("redisEvent", customerLocationSchema))\
    .select(col("customerLocation.*"))\
    .createOrReplaceTempView("customerLocation")

# query data from temp view
customerStreamingDF = spark.sql("select accountNumber as customerAccountNumber, location as homeLocation, birthDay from Customer")
...
customerHomeStreamingDF = relevantCustomerFieldsStreamingDF.join(customerLocationStreamingDF, 
                                                            expr(""" customerAccountNumber = accountNumber
                                                            """)
                                                           )
customerHomeStreamingDF.writeStream.outputMode("append").format("console").start().awaitTermination()
